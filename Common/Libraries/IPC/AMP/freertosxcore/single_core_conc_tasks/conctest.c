...
void vStartConcTasks( unsigned portBASE_TYPE uxPriority ) {

	xTaskCreate( vConcTestMonitor, ( signed char * ) "T1", configMINIMAL_STACK_SIZE, NULL, uxPriority, NULL );
	xTaskCreate( vConcTest0, ( signed char * ) "T1", configMINIMAL_STACK_SIZE, NULL, uxPriority, NULL );
	xTaskCreate( vConcTest1, ( signed char * ) "T2", configMINIMAL_STACK_SIZE, NULL, uxPriority, NULL );
...
static portTASK_FUNCTION( vConcTestMonitor, pvParameters )
{
	...
	for (;;) {
		...
		if ((calc1Done == 10000000) && (calc2Done == 10000000)) {
			...
			vTaskDelay( CHECK_TASK_DELAY_PERIOD );
